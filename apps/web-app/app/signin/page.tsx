  ...
  const features = [
    {
      ...
      title: t.signin.features.transcription.title,
      description: t.signin.features.transcription.description
    },
    {
      icon: <Clock className="w-8 h-8 text-[#4fb3c1]" />,
      title: t.signin.features.timeSaving.title,
      description: t.signin.features.timeSaving.description
    },
    {
      icon: <Users className="w-8 h-8 text-[#7dd8e3]" />,
      title: t.signin.features.actionTracking.title,
      description: t.signin.features.actionTracking.description
    },
    {
      icon: <Zap className="w-8 h-8 text-[#3da6b5]" />,
      title: t.signin.features.insights.title,
      description: t.signin.features.insights.description
    }
  ];

  const stats = [
    { number: "10K+", label: t.signin.stats.meetings },
    { number: "2h", label: t.signin.stats.timeSaved },
    { number: "98%", label: t.signin.stats.accuracy }
  ];
      ...
      <div className="relative z-10 flex min-h-screen">
        ...
        <div className="hidden lg:flex lg:w-1/2 xl:w-3/5 flex-col justify-center p-12 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white relative">
          ...
          <div className="space-y-6">
            <h3 className="text-2xl font-semibold mb-6">{t.signin.coreFeatures}</h3>
        ...
        <div className="w-full lg:w-1/2 xl:w-2/5 flex items-center justify-center p-8">
          <div className="w-full max-w-md">
            ...
            <div className="lg:hidden text-center mb-8">
              ...
              <p className="text-slate-600">{t.signin.subtitle}</p>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8 border border-slate-200 relative overflow-hidden">
              ...
              <div className="text-center mb-8">
                <h2 className="text-2xl font-bold text-slate-800 mb-2">{t.signin.welcomeBack}</h2>
                <p className="text-slate-600">{t.signin.startJourney}</p>
              </div>

              {/* 功能预览 - 移动端 */}
              <div className="lg:hidden mb-6">
                <div className="bg-gradient-to-r from-[#61d0de]/5 to-[#7dd8e3]/5 rounded-xl p-4 mb-4 border border-[#61d0de]/10">
                  <h3 className="font-semibold text-slate-800 mb-3">{t.signin.experienceNow}</h3>
                  <div className="space-y-2 text-sm text-slate-600">
                    <div className="flex items-center">
                      <CheckCircle className="w-4 h-4 text-[#61d0de] mr-2" />
                      <span>{t.signin.featureList.transcription}</span>
                    </div>
                    <div className="flex items-center">
                      <CheckCircle className="w-4 h-4 text-[#61d0de] mr-2" />
                      <span>{t.signin.featureList.taskExtraction}</span>
                    </div>
                    <div className="flex items-center">
                      <CheckCircle className="w-4 h-4 text-[#61d0de] mr-2" />
                      <span>{t.signin.featureList.insights}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {loginLoading ? (
                  <div className="flex items-center justify-center gap-2 py-3">
                    <LoadingSpinner />
                    <span className="text-slate-600">{t.signin.signingIn}</span>
                  </div>
                ) : (
                  <Button
              ...
              <div className="text-center mt-6">
                <p className="text-xs text-slate-500">
                  {t.signin.loginAgreement}
                  <a href="#" className="text-[#61d0de] hover:text-[#4fb3c1] hover:underline transition-colors">{t.signin.terms}</a>
                  {locale === 'zh' ? '和' : ' and '}
                  <a href="#" className="text-[#61d0de] hover:text-[#4fb3c1] hover:underline transition-colors">{t.signin.privacy}</a>
                </p>
              </div>
            </div>
